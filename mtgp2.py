...
class FeasibleProbSampling(SamplingStrategy):
    ...
    def forward(
        ...
        constraints = self.C_models.posterior(X, observation_noise=observation_noise).rsample(sample_shape=torch.Size([100]))

        feasible_count = torch.all(constraints <= 0, dim=2).sum(dim=0)
        _, idx = torch.topk(feasible_count, num_samples)
        return X[idx, :]
...
def generate_batch(
    ...
    feasible = torch.any(torch.all(C <= 0, dim=1))
    if feasible:
    ...
    if feasible:
        ...
        sampling = FeasibleProbSampling(C_models=C_models)
        with torch.no_grad():
