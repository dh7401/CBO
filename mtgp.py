...
from mopta import mopta_evaluate1
...
class ExtendedThompsonSampling(SamplingStrategy):
    ...
    def forward(
        ...
        samples = self.model.posterior(X, observation_noise=observation_noise).rsample(sample_shape=torch.Size([num_samples]))
        constraints = self.C_models.posterior(X, observation_noise=observation_noise).rsample(sample_shape=torch.Size([num_samples]))
...
Y_turbo = torch.tensor(
    [-mopta_evaluate1(x)[0] for x in X_turbo], dtype=dtype, device=device
).unsqueeze(-1)
C_turbo = torch.stack([mopta_evaluate1(x)[1:] for x in X_turbo]).to(device)
...
while not state.restart_triggered:  # Run until TuRBO converges
    ...
    C_models = KroneckerMultiTaskGP(X_turbo, C_turbo)
    C_mlls = ExactMarginalLogLikelihood(C_models.likelihood, C_models)

    # Do the fitting and acquisition function optimization inside the Cholesky context
    with gpytorch.settings.max_cholesky_size(max_cholesky_size):
        # Fit the model
        fit_gpytorch_model(mll)
        fit_gpytorch_torch(C_mlls, options={"maxiter": 3, "lr": 0.01, "disp": False})
    ...
    Y_next = torch.tensor([-mopta_evaluate1(x)[0] for x in X_next], dtype=dtype, device=device).unsqueeze(-1)
    C_next = torch.stack([mopta_evaluate1(x)[1:] for x in X_next]).to(device)
